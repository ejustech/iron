 ...
 *
 */
public class Result31Action extends EventDispatchAction {
	/*
	 * Generated Methods
	 */
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			list = (ArrayList<Result3FormBean>) session.getAttribute("CHUMENG1LIST"); // 此时取出来的是Object,
																						// 需要强转
			// 定义导出excel名字
			String fileName = "生产数据统计表-除锰（产量）.xls";
			fileName = new String(fileName.getBytes("UTF-8"), "utf-8");
			//取得检索日期
			String riqi = (String) session.getAttribute("RIQI");
			// 导出excel到服务器
			Excel.exportResult31Excel(list, fileName, riqi);
			// 下载excel到本地
